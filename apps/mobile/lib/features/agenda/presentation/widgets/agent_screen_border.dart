...
class _FastShaderBeamPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    if (option.name.contains('SARA')) {
      // Degradado Ciruela y Malva de SARA: analogía al isotipo dinámico
      gradientColors = [
        Colors.transparent,
        Colors.transparent,
        const Color(0xFF6B4F73).withValues(alpha: 0.15), // Cola ciruela suave
        const Color(0xFF6B4F73).withValues(alpha: 0.60), // Ciruela institucional
        const Color(0xFF9E80A8).withValues(alpha: 0.85), // Malva intermedio
        const Color(0xFFC5AED0).withValues(alpha: 0.95), // Malva brillante
    ...
    } else if (option.isMulticolor) {
      // Degradado multicolor Google / Gemini que fluye en el cuerpo del lazo
      gradientColors = [
        Colors.transparent,
        Colors.transparent,
